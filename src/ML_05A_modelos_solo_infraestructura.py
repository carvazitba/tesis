...
Descripción:
Experimento de ablación A: entrena modelos usando ÚNICAMENTE variables de
infraestructura urbana y geométricas, excluyendo toda variable delictiva
y toda variable temporal.

Formulación de la hipótesis que este experimento pone a prueba:
    ¿La geografía urbana, por sí sola, sabe dónde se concentra el delito?

El modelo resultante asigna el mismo score a una celda en cualquier mes y
franja horaria: mide discriminación puramente ESPACIAL. La métrica central
de comparación contra el modelo completo (ML_05) es el ROC-AUC, que es
...
Salidas:
    ...
    outputs/resumen_roc_auc_solo_infraestructura.csv
...
NOTA METODOLÓGICA:
    - La validación usa splits temporales por MES COMPLETO (idéntica a
      ML_05 y ML_06), de modo que los ROC-AUC son directamente comparables
      con los del modelo completo.
    - XGBoost compensa el desbalance con scale_pos_weight calculado por
      fold, igual que en ML_05.
"""
...
SHAP_DIR = OUTPUT_DIR / "shap_ML05A"
...
OUTPUT_RESUMEN_AUC = OUTPUT_DIR / "resumen_roc_auc_solo_infraestructura.csv"
...
# Estratificada por target. Como las features son constantes por celda,
# el ranking global es muy estable con este tamaño.
N_SAMPLE_SHAP = 50_000
N_TOP_DEPENDENCE = 4

# Selección PURISTA: solo infraestructura urbana y geometría de la celda.
# Sin variables delictivas, sin variables temporales (anio, mes_num,
# trimestre, semestre, franja). El experimento mide qué sabe la geografía
# urbana por sí sola.
...
# (defensa en profundidad contra fugas de información delictiva o temporal).
PATRONES_PROHIBIDOS = [
    "delito", "hotspot", "promedio", "historico", "lag", "rolling",
    "umbral", "anio", "mes", "trimestre", "semestre", "franja",
    "fecha", "grid_id",
]
...
def seleccionar_features_infraestructura(df: pd.DataFrame) -> list:
    """
    Selecciona únicamente variables de infraestructura (cant_*, dist_min_*)
    y geométricas. Aplica luego los patrones prohibidos como verificación
    adicional: si por error una candidata contuviera información delictiva
    o temporal, queda excluida.
    """
    candidatas = []

    for col in df.columns:
        if col.startswith("cant_") or col.startswith("dist_min_"):
            candidatas.append(col)
        elif col in FEATURES_GEOMETRICAS:
            candidatas.append(col)
...
def generar_splits_temporales_por_mes(df: pd.DataFrame, n_splits: int = 5):
    ...
    no por filas. Idéntica a ML_05 y ML_06: los resultados son
...
def analisis_shap_xgboost(modelo, X: pd.DataFrame, y: pd.Series):
    """
    Genera el análisis SHAP del modelo XGBoost solo-infraestructura.

    Salidas (en outputs/shap_ML05A/):
        shap_summary_bar_solo_infraestructura.png
        shap_summary_beeswarm_solo_infraestructura.png
        shap_dependence_solo_infraestructura_<feature>.png (top 4)
        shap_importance_solo_infraestructura.csv

    El CSV permite comparar el ranking interno de las variables de
    infraestructura cuando están solas contra el ranking que tenían
    dentro del modelo completo (ML_07).
    """
    ...
    print("\n📊 Análisis SHAP del modelo solo-infraestructura (XGBoost)...")
    ...
    plt.title("Importancia global |SHAP| — solo infraestructura", fontsize=13)
    plt.tight_layout()
    ruta = SHAP_DIR / "shap_summary_bar_solo_infraestructura.png"
    ...
    plt.title("SHAP summary — solo infraestructura", fontsize=13)
    plt.tight_layout()
    ruta = SHAP_DIR / "shap_summary_beeswarm_solo_infraestructura.png"
    plt.savefig(ruta, dpi=300, bbox_inches="tight")
    plt.close()
    print(f"   ✓ {ruta.name}")
    ...
    for idx in top_idx:
        ...
        plt.title(f"SHAP dependence — solo infraestructura — {feature}",
                  fontsize=12)
        plt.tight_layout()

        nombre_archivo = feature.replace("/", "_").replace(" ", "_")
        ruta = SHAP_DIR / f"shap_dependence_solo_infraestructura_{nombre_archivo}.png"
        plt.savefig(ruta, dpi=300, bbox_inches="tight")
        plt.close()
        print(f"   ✓ {ruta.name}")
    ...
    ruta = SHAP_DIR / "shap_importance_solo_infraestructura.csv"
    df_imp.to_csv(ruta, index=False)
    print(f"   ✓ {ruta.name}")

    print("\n   Top 10 features (solo infraestructura):")
...
def main():
    print("=" * 75)
    print("EXPERIMENTO DE ABLACIÓN A — SOLO INFRAESTRUCTURA (Y GEOMÉTRICAS)")
    print("=" * 75)
    ...
    # de comparación contra el modelo completo de ML_05.
    resumen_auc = (
        resultados_folds
    ...
    print("\n📊 ROC-AUC promedio por modelo (comparar contra ML_05):")
    ...
    print(f"\n📌 NOTA (para la tesis):")
    print(f"  La métrica de comparación contra el modelo completo es ROC-AUC.")
